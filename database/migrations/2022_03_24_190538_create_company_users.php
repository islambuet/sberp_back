...
class CreateCompanyUsers extends Migration
{
    ...
    public function up()
    {
        Schema::create(TABLE_COMPANY_USERS, function (Blueprint $table) {
            ...
            $table->enum('reason_status_active', [COMPANY_USER_STATUS_ACTIVE_ADMIN, COMPANY_USER_STATUS_ACTIVE_INVITATION, COMPANY_USER_STATUS_ACTIVE_REQUEST]);            
            $table->integer('status_active_id')->default(0);
            $table->enum('reason_status_inactive', [COMPANY_USER_STATUS_INACTIVE_ADMIN, COMPANY_USER_STATUS_INACTIVE_REMOVE, COMPANY_USER_STATUS_INACTIVE_LEAVE]);            
